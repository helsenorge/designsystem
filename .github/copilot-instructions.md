...
Requires Node `^24.0.0 || ^26.0.0` and npm `11.17.0'`.
...
Release prep, commit, and publish (PR-based flow):

```bash
npm run release:prepare         # build + validate + test + pack --dry-run
npm run release:commit-and-push # commit version bump, prepend the new CHANGELOG section, push the branch for a PR
npm run publish:latest          # publish all three packages to npmjs.org
npm run publish:beta            # publish under the "beta" dist-tag
```

`release:ensure-clean` aborts if the working tree is dirty. Releases now go through a PR: `release:commit-and-push` pushes only the branch
(no tag). After the PR is merged, the `vX.Y.Z` tag is created on `master` (`npm run v:tag` + push) — pushing that tag drives publishing CI
in `Pipelines/`. The CHANGELOG is generated by `scripts/generate-changelog.ts`, which **prepends** only the newest section (it never
regenerates history), so a release section can never be swallowed by an unreachable/orphaned tag.
...
Commits follow Conventional Commits (`@commitlint/config-conventional`). `CHANGELOG.md` is generated by `scripts/generate-changelog.ts` (the
programmatic `conventional-changelog` v7 API) via `npm run changelog` during the release flow. It **prepends** only the newest section and
never rewrites existing history — do not edit it by hand.
