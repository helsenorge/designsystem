...
const NoAccess: React.FC<SvgPathProps> = ({ size, isHovered, onColor: mode }: SvgPathProps): React.ReactElement => {
  const normal = (
    <path
      fillRule={'evenodd'}
  ...
  const xxSmall = (
    <>
      {mode === FormOnColor.ondark && <circle cx="24" cy="24" r="17" fill="white" />}
      <path
        fillRule={'evenodd'}
