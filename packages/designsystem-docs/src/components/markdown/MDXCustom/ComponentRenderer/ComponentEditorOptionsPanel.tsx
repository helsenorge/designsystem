...
function StyledEditorButton({className, onClick, icon, hoverIcon, isActive = false, isPanelOpen = false}: EditorButtonProps) {
    const iconRef = useRef(null);
    return (
        <button onMouseEnter={() => iconRef.current.src = `${hoverIcon}`} onMouseLeave={() => iconRef.current.src = `${icon}`} onClick={onClick} className={className}>
